...
__addon_id__= 'plugin.audio.bgradio_online'
__Addon = xbmcaddon.Addon(__addon_id__)
musicfolder = xbmc.translatePath(__Addon.getAddonInfo('path') + "/resources/musicfolder.png")
item = xbmc.translatePath(__Addon.getAddonInfo('path') + "/resources/item.png")
...
REF = base64.b64decode("aHR0cDovL2JncmFkaW8tb25saW5lLmNvbS8=")
...
def CATEGORIES():
        ...
        data=response.read()
        response.close()
        match = re.compile('th><a href="(.+?)" target="Radio"><img.*data-src="(.+?)".*<br />(.+?)<').findall(data)
...
def PLAY(name,url,iconimage):
        ...
        for radioname,link in match:
         ...
         li.setInfo('video', { 'title': radioname })
         try:
